...
#f = open(file_name, "w")
#f.write("start_time,cur_time,pod_name,num_iteration,total_iteration,loss,gain,epoch_time,cpu\n")
batch_size = 128
no_cuda = False
seed = 1
log_inverval = 10
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epoches+1):
        ...
        logging.warning("{},{},{},{},{},{},{},{},{}".format(start_time, str(time.time()),
                                                      pod_name, str(epoch),
                                                      str(epoches), str(test_loss),
                                                      str(last_loss - test_loss), str(time.time() - cur_time),
                                                      str(p.cpu_percent())))
        #f.write("{},{},{},{},{},{},{},{},{}\n".format(start_time,str(time.time()),
         #                                         pod_name,str(epoch),
         #                                         str(epoches),str(test_loss),
         #                                         str(last_loss-test_loss),str(time.time()-cur_time),str(p.cpu_percent())))
        #f.flush()
        cur_time = time.time()
        last_loss = test_loss
        with torch.no_grad():
